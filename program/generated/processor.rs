...
/// 2. `[writable]` video: [VideoNftMetadata] 
...
/// - category: [String] 
	pub fn process_mint(
		...
		// Derive PDAs
		let (video_pubkey, video_bump) = Pubkey::find_program_address(
			&[b"video", mint_info.key.as_ref()],
			program_id,
		);
		...
		if *video_info.key != video_pubkey {
		...
		let space = VideoNftMetadata::LEN;
		let rent = Rent::get()?;
		let rent_minimum_balance = rent.minimum_balance(space);
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), video_info.clone()],
			&[&[b"video", mint_info.key.as_ref(), &[video_bump]]],
		)?;
		...
		if video_info.data_len() != VideoNftMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let video = &mut AccountPDA::new(
			&video_info,
			try_from_slice_unchecked::<VideoNftMetadata>(&video_info.data.borrow()).unwrap(),
			video_bump,
		);
		...
		video.data.serialize(&mut &mut video_info.data.borrow_mut()[..])?;
