...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let video_url: String = Default::default();
		let publisher: String = Default::default();
		...
		let (video_pubkey, _) = Pubkey::find_program_address(
			&[b"video", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(video_pubkey, false),
