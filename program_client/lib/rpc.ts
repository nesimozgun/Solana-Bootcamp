 ...
 * 2. `[writable]` video: {@link VideoNftMetadata} 
 ...
 *
 * Data:
 * - video_url: {@link string} 
 * - publisher: {@link string} 
...
export const mint = (args: MintArgs): TransactionInstruction => {
    ...
    const [videoPubkey] = pda.deriveVideoNftMetadataPDA({
        mint: args.mint,
    }, _programId);
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            {pubkey: videoPubkey, isSigner: false, isWritable: true},
...
export const getVideoNftMetadata = async (
    publicKey: PublicKey,
    commitmentOrConfig: Commitment | GetAccountInfoConfig | undefined = "processed"
): Promise<T.VideoNftMetadata | undefined> => {
    ...
    return T.decodeVideoNftMetadata(deserialize(T.VideoNftMetadataSchema, buffer.data) as Record<string, unknown>);
}
